...
<body>
        ...
        <tbody>
            @foreach ($rates as $rate)
                <tr>
                    ...
                    <td>{{ $rate->comment}}</td>
                    <td>{{ $rate->stare }}</td>
                    <td><a href="{{ Route('rates.edit', ['rate' => $rate->id]) }}"><img style="width: 30px"
                                ...
                                alt=""></a></td>
                    <td><a href="{{Route('rates.destroy',['rate'=>$rate->id])}}"><img style="width: 30px" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTgxgk2JJECcveyd80tbCtE3TwsNj-WlmUApw&usqp=CAU" alt=""></a></td>
                </tr>
            @endforeach
        </tbody>
    </table>
    <br>
    <button>
        <a href="{{route('rates.create')}}">Add rate</a>
    </button>
</body>
